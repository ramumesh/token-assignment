...
const SingleToken = () => {
    ...
    return <div className="single-token-container">
        <DigitChooser onDigitsSelected={onDigitsSelected} />


        <div className="generate-token">
            <div className="flex-container">
                <button className="app-button" disabled={digits.length < 2} onClick={onGenerateToken}>{STRINGS.GENERATE_TOKEN}</button>
                <button className="app-button" disabled={!token || digits.length < 2} onClick={onValidateToken}>{STRINGS.VALIDATE_TOKEN}</button>
            </div>
            <div className="flex-container">
                <Token value={token} />
                <div className="message">{message ? message : STRINGS.DUMMY_VALIDATION_MESSAGE}</div>
            </div>
        </div>
